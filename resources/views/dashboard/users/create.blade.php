...
<div class="col-md-10 col-lg-8">
  ...
  @if(session("error"))
    ...
    @endif
  <form id="form" class="p-2" action="/dashboard/users/" method="post" accept-charset="utf-8" class="text-capitalize" enctype="multipart/form-data">
    @csrf
    ...
    <div style="width: 12rem; height: 12rem" class="profile-frame border rounded mb-3 position-relative">
      <img data-src="{{ url('post-images/no-pp.jpg') }}" id="preview" style="height: 100%;width:100%" class="rounded-circle" src="{{ url('post-images/no-pp.jpg') }}" alt="" />
    ...
    <div class="form-floating mb-3">
      <input type="text" name="username" class="form-control @error('username') is-invalid @enderror" id="floatingInput" placeholder="Example@example.com" value="{{ old('username') }}">
    ...
    <div class="form-floating mb-3">
      <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" id="floatingInput" placeholder="Example@example.com" value="{{ old('email') }}">
    ...
    <div class="form-floating mb-3">
      <input type="password" name="password" class="form-control @error('password') is-invalid @enderror" id="floatingInput" placeholder="password" value="">
...
<script type="text/javascript" charset="utf-8">
  $("#rolesSelect").on("change", function() {
    ...
    if (!roles.includes("1") && !roles.includes("2") && !roles.includes("3")) {
       return myConfirm("shit, really?", "user will lose all roles!")
    }
    for (var i = 1; i <= allRoles.length; i++) {
      let role = allRoles[i - 1]
      if (!roles.includes(`${i}`)) {
        return myConfirm("are you sure?", `user will lose ${role} roles!`)
      }
    }
  })
</script>
